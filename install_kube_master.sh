...
usage() {
    ...
    echo "Options:"
    echo "  --pod-cidr=CIDR               CIDR du réseau des Pods (défaut: $POD_CIDR)"
    echo "  --k8s-version=VERSION         Version de Kubernetes (défaut: $K8S_VERSION)"
    echo "  --control-plane-endpoint=EP   Endpoint du control-plane (défaut: $CONTROL_PLANE_ENDPOINT)"
...
TEMP=$(getopt \
  -o h \
  --long help,pod-cidr:,k8s-version:,control-plane-endpoint: \
  -n "$0" -- "$@")
...
echo "Création du fichier /etc/containerd/config.toml..."
...
echo "Initialisation du cluster Kubernetes avec Pod CIDR : $POD_CIDR et Control-Plane Endpoint : $CONTROL_PLANE_ENDPOINT"
kubeadm init --upload-certs --pod-network-cidr="$POD_CIDR" --control-plane-endpoint="$CONTROL_PLANE_ENDPOINT"
...
echo "Pour utiliser kubectl en root, exécutez :"
